...
export default function ReligionForm() {
    const initialMasterreligionState = {
      ...
      const [masterreligion, setMasterreligion] = useState(initialMasterreligionState);
      const [submitted, setSubmitted] = useState(false);

      const handleInputChange = event => {
        const { name, value } = event.target;
        setMasterreligion({ ...masterreligion, [name]: value });
      };
      
      const saveMasterreligion = () => {
        var data = {
          name: masterreligion.name,
        };
    
        MasterreligionDataService.create(data)
          .then(response => {
            setMasterreligion({
              id: response.data.id,
                    ...
                    <div>    
                        <div className="w-full lg:w-12/12 mb-10 font-light">
                            <Input
                                ...
                                id="name"
                                required
                                value={masterreligion.name}
                                onChange={handleInputChange}
                                name="name"
                            />
                        </div>
                        <Button onClick={saveMasterreligion} className="btn btn-success" color="blue">
                             Submit
                        </Button>
                    </div>        
                     )}
